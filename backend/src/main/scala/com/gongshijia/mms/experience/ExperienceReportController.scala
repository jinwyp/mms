...
import com.gongshijia.mms.core.{Core, MongoSupport, MongoTestSupport, Neo4jSupport}
...
/**
  ...
  */
trait ExperienceReportController extends Core with MongoSupport with Neo4jSupport with LoginService with MongoTestSupport {
  ...
  def findByIdAndOpenId(fromOpenId: String, currentOpenId: String, reportId: String): Future[ExperienceReport] = {
    val collection: MongoCollection[ExperienceReport] = mongoDb.getCollection(reportCollection)
    for {
      result <- createFriendRelationShip(fromOpenId = fromOpenId, toOpenId = currentOpenId, expId = reportId)
  ...
  def updateSureSignInfo(openId: String, reportId: String, signInfoId: String): Future[Boolean] = {
    ...
    collection.find(and(equal("_id", new ObjectId(reportId)), equal("openid", openId))).first().toFuture() map { report =>
      ...
      collection.updateOne(equal("_id", new ObjectId(reportId)), set("checked", 1)).toFuture().isCompleted
    }
  }
}
